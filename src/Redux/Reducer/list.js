...
const tasks = (state = newState, action) => {
  switch (action.type) {
    ...
    case types.changeStatus:
      state[action.index][action.indexitem].status =
        !state[action.index][action.indexitem].status;
      return [...state];
    ...
    case types.changStatusTrue:
      state[action.checkObj[0].index][
        action.checkObj[0].indexItem
      ].status = true;
      state[action.checkObj[1].index][
        action.checkObj[1].indexItem
      ].status = true;
      return [...state];
